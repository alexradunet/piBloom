...
The reusable service behavior lives in `nix/modules/dav-server.nix`. Production switching happens from `/root/nazar` with `nix flake check --no-build` and `nix run .#switch-dav-server` after committing monorepo changes.
