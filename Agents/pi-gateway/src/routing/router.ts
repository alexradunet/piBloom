...
export class Router {
  ...
  private async handleMessageInner(
    ...
    try {
      const prompt = this.buildPrompt(msg.channel, text);
  ...
  private buildPrompt(channel: string, userText: string): string {
    return [
      `You are replying through the ${channel} gateway module.`,
