...
If bootstrap fails with the GRUB assertion about `boot.loader.grub.devices`, the usual cause is not the plain OVH install itself.
It usually means `/etc/nixos/configuration.nix` was generated separately with `nixos-generate-config` and is conflicting with the NixPI bootstrap flow.
