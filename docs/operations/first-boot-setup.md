...
Bootstrap also keeps the `root` account available for local console login and generates recovery passwords for both the primary user and `root`, prints them during bootstrap, and stores them at `/root/nixpi-bootstrap-passwords.txt` with root-only permissions unless you pass explicit passwords yourself.
Save those passwords immediately. They are for OVH KVM or rescue fallback only; SSH still stays key-only with root login disabled over SSH.
...
then a generic `/etc/nixos/configuration.nix` is probably still present from `nixos-generate-config`.
Keep `/etc/nixos/hardware-configuration.nix`, move the generic configuration aside, and rerun bootstrap with `--force`:
