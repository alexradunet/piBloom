...
write_host_module() {
	...
	local timezone="$4"
	local keyboard="$5"
	local primary_user_password_hash="$6"
	local root_password_hash="$7"
	shift 7
	...
	local primary_user_password_block="" root_password_block=""
	...
	if [[ -n "$primary_user_password_hash" ]]; then
		primary_user_password_block=$'\n'"  users.users.${primary_user}.hashedPassword = \"$(escape_nix_string "$primary_user_password_hash")\";"
	fi

	if [[ -n "$root_password_hash" ]]; then
		root_password_block=$'\n'"  users.users.root.hashedPassword = \"$(escape_nix_string "$root_password_hash")\";"
	fi
