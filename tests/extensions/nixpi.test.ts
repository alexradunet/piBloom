...
import { ensureNixPi, getPackageDir, handleNixPiStatus } from "../../core/pi/extensions/nixpi/actions.js";
...
describe("nixpi extension", () => {
	it("does not export unused skill helper actions from the nixpi module", async () => {
		const mod = await import("../../core/pi/extensions/nixpi/actions.js");
		expect("handleSkillCreate" in mod).toBe(false);
		expect("handleSkillList" in mod).toBe(false);
	});
