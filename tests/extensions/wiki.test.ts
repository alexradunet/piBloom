...
`;
	...
	it("deriveWikiMetaArtifacts builds pure metadata outputs from pages and events", () => {
		const content = `---
title: Test Page
type: concept
status: active
updated: "2026-04-10"
source_ids: []
summary: A page used for pure metadata derivation
---
# Test Page
...
`;
		...
		expect(artifacts.registry.pages).toHaveLength(1);
		expect(artifacts.backlinks.byPath["pages/test-page.md"]?.outbound).toEqual([]);
		expect(artifacts.index).toContain("# Wiki Index");
		expect(artifacts.index).toContain("Test Page");
		expect(artifacts.log).toContain("# Wiki Log");
		expect(artifacts.log).toContain("Rebuilt wiki metadata");
	});
...
`;
	...
	it("appendEvent adds an event and readEvents returns it; second append gives 2 events", () => {
		...
		const appendResult1 = appendEvent(tmpDir, event1);
		expect(appendResult1).toBeUndefined();
		const events1 = readEvents(tmpDir);
		expect(events1).toHaveLength(1);
		expect(events1[0].kind).toBe("capture");
		expect(events1[0].title).toBe("First Capture");

		const appendResult2 = appendEvent(tmpDir, event2);
		expect(appendResult2).toBeUndefined();
		const events2 = readEvents(tmpDir);
...
describe("captureText / captureFile", () => {
	...
	it("captureFile rejects pdf inputs until extraction is implemented", () => {
		const pdfFile = path.join(wikiRoot, "input.pdf");
		writeFileSync(pdfFile, Buffer.from([0x25, 0x50, 0x44, 0x46, 0x00, 0xff]));

		const result = captureFile(wikiRoot, pdfFile);
		...
		expect(existsSync(path.join(wikiRoot, "pages", "sources", "SRC-2025-06-15-001.md"))).toBe(false);
	});

	it("captureFile rejects binary files that are not valid utf-8 text", () => {
		const binaryFile = path.join(wikiRoot, "input.bin");
		writeFileSync(binaryFile, Buffer.from([0xff, 0xfe, 0x00, 0x01]));

		const result = captureFile(wikiRoot, binaryFile);
		...
		expect(readdirSync(path.join(wikiRoot, "raw"))).toHaveLength(0);
	});
});
...
describe("handleEnsurePage", () => {
	...
	it("creates new page when none exists (details.created === true, file exists on disk)", () => {
		...
		if (result.isOk()) {
			...
			if (!result.value.details?.resolved || result.value.details.conflict) {
				throw new Error("Expected created page to return a concrete page path");
			}
			const relPath = result.value.details.path;
...
`;
		...
		if (result.isOk()) {
			if (!result.value.details?.resolved || result.value.details.conflict) {
				throw new Error("Expected alias resolution to return a concrete page path");
			}
